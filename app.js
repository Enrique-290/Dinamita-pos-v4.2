...
// Ticket print fix
const TicketFix={ printVisible(){ const prev=document.getElementById('ticketPreview'); if(prev){ prev.style.display='block'; setTimeout(()=>{ window.print(); setTimeout(()=>prev.style.display='', 100); }, 50);} else { window.print(); } } };
...
Historial.cancelarVenta = function(folio){
  ...
  if(v.status==='Cancelada'){ alert('Ya está cancelada'); return; }
...
window.Compras = {
  ...
  init(){
    ...
    const inp=document.getElementById('cpBuscar');
    if(inp){ inp.onkeydown=(e)=>{ if(e.key==='Enter'){ Compras.buscarYAgregar(inp.value.trim()); inp.value=''; } }; }
  ...
  guardar(){
    ...
    compra.items.forEach(it=>{
      const p=(state.products||[]).find(x=>x.sku===it.sku);
      if(p){
        const stockPrev=p.stock||0; const costoPrev=p.costo||0;
